...
# [2] 상태 초기화 (중복 저장 방지 및 자동저장 체크용)
if 'submitted' not in st.session_state:
    st.session_state.submitted = False

# [3] CSS 고정: 모든 버튼 스타일 + 팝업 'X' 버튼 숨기기
st.markdown("""
    <style>
    /* 1. 사이드바 제출 버튼 (파란색, 대형) */
    ...
    /* ★핵심★ 4. 팝업창(Dialog) 우측 상단 'X' 닫기 버튼 숨기기 */
    button[aria-label="Close"] {
        display: none !important;
    }

    /* 5. 비활성화 체크박스 글자색 검정 고정 */
...
# [4] 이수 기준 변수 고정 (절대 변경 금지)
TARGET_TOTAL_HOURS = 17.0
TARGET_MANDATORY_HOURS = 3.5
TARGET_COMMON_HOURS = 6.5
TARGET_SPECIAL_HOURS = 7.0
...
# [6] 최종 확인 팝업창 (X 버튼 제거 및 자동저장 로직)
@st.dialog("📝 최종 수강 신청 신청 완료", width="large")
def show_confirmation_dialog(data, user_info):
    # 팝업이 열리는 순간 즉시 자동 저장 실행 (1회)
    ...
    st.success("🎉 신청 정보가 자동으로 서버에 기록되었습니다.")
    ...
    st.divider()
    st.info("✅ 신청 완료. 소상공인지식배움터에 접속하여 강의명 검색 후 수강하시기 바랍니다.")
    st.error("⚠️ 촬영을 완료하셨다면 브라우저 탭을 닫거나 종료해 주세요.")
...
if os.path.exists(EXCEL_PATH):
    ...
    u_name = st.sidebar.text_input("성함", placeholder="이름 입력")
    u_biz = st.sidebar.text_input("업체명", placeholder="사업장명 입력")
    u_cat = st.sidebar.selectbox("구분", ["경영개선", "재창업"])
    u_phone = st.sidebar.text_input("전화번호", placeholder="010-0000-0000")
    u_email = st.sidebar.text_input("이메일", placeholder="example@mail.com")
    ...
    if st.session_state.submitted:
        st.sidebar.info("✅ 전송 및 신청이 완료되었습니다.")
        ...
        if st.sidebar.button("최종 수강목록 확인 및 제출", type="primary"):
            if u_name and u_biz and u_phone:
                show_confirmation_dialog(r_df, {'name': u_name, 'biz': u_biz, 'category': u_cat, 'phone': u_phone, 'email': u_email})
            else: st.sidebar.error("기본 정보(성함, 업체명, 연락처)를 입력해 주세요.")
